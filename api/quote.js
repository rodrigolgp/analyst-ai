export default async function handler(req, res) {
  ...
  // ── Modo historico: busca preco de fechamento de uma data especifica ──
  // Usado para comparacoes de periodo (ex: Ibovespa desde inicio do teste)
  if (data) {
    try {
      const period1 = Math.floor(new Date(data + "T00:00:00Z").getTime() / 1000);
      const period2 = period1 + 86400 * 10;
      const sym = ticker.startsWith("^") ? ticker.replace("^","%5E") : ticker;
